...
var normalizeStatus = /* @__PURE__ */ __name((status) => {
  if (status === "approved") {
...
var worker = {
  async fetch(request, env) {
    ...
    const paymentsResult = await supabase.from("payments").upsert({
    ...
    try {
      ...
      const bookingUpdate = await supabase.from("bookings").update({ status: booking }).eq("id", payload.booking_id);
      if (bookingUpdate.error) {
        console.error("Booking update failed", bookingUpdate.error);
        throw new Error("Error updating booking");
      }
      const intentsUpdate = await supabase.from("payment_intents").update({ status: payment }).eq("booking_id", payload.booking_id);
      ...
      return jsonResponse({ message: error instanceof Error ? error.message : "Server error" }, { status: 500 });
    }
  }
};
...
// .wrangler/tmp/bundle-ccRlpV/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ccRlpV/middleware-loader.entry.ts
