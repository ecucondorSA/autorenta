...
function initSentry(request, env, ctx) {
  ...
  sentryInstance = {
    captureException: /* @__PURE__ */ __name(() => void 0, "captureException"),
    captureMessage: /* @__PURE__ */ __name(() => void 0, "captureMessage"),
    addBreadcrumb: /* @__PURE__ */ __name(() => {
    }, "addBreadcrumb"),
    setTags: /* @__PURE__ */ __name(() => {
    }, "setTags"),
    setUser: /* @__PURE__ */ __name(() => {
    }, "setUser"),
...
function addBreadcrumb(message, category = "default", data, level = "info") {
  const sentry = getSentry();
  if (!sentry) {
    return;
  }
...
async function withSentry(request, env, ctx, handler) {
...
// dist/index.js
var jsonResponse = /* @__PURE__ */ __name((data, init = {}) => new Response(JSON.stringify(data), {
  headers: {
    "content-type": "application/json; charset=UTF-8"
  },
  ...init
}), "jsonResponse");
var getSupabaseAdminClient = /* @__PURE__ */ __name((env) => {
  ...
  return createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY, {
    ...
    global: {
      fetch: /* @__PURE__ */ __name((input, init) => fetch(input, init), "fetch")
    }
  });
}, "getSupabaseAdminClient");
var normalizeMockStatus = /* @__PURE__ */ __name((status) => {
  if (status === "approved") {
    return { payment: "completed", booking: "confirmed" };
  }
  return { payment: "failed", booking: "cancelled" };
}, "normalizeMockStatus");
var normalizeMPStatus = /* @__PURE__ */ __name((status) => {
  switch (status) {
    case "approved":
      return { payment: "completed", booking: "confirmed" };
    case "rejected":
...
var parseSignatureHeader = /* @__PURE__ */ __name((signature) => {
...
var verifyMercadoPagoSignature = /* @__PURE__ */ __name(async (params) => {
  ...
  try {
    ...
    const computedHash = Array.from(new Uint8Array(signatureBuffer)).map((b) => b.toString(16).padStart(2, "0")).join("");
...
var getMercadoPagoPaymentDetails = /* @__PURE__ */ __name(async (paymentId, accessToken) => {
...
var processMockWebhook = /* @__PURE__ */ __name(async (payload, supabase, env, log) => {
  ...
  try {
    ...
    const { error: bookingError } = await supabase.from("bookings").update({ status: booking }).eq("id", payload.booking_id);
    if (bookingError) {
      console.error("Booking update failed:", bookingError);
      throw new Error("Error updating booking");
    }
    const { error: intentError } = await supabase.from("payment_intents").update({ status: payment }).eq("booking_id", payload.booking_id);
    if (intentError) {
...
var processMercadoPagoWebhook = /* @__PURE__ */ __name(async (payload, supabase, env, options, log) => {
  ...
  const signatureValid = await verifyMercadoPagoSignature({
    paymentId,
    signatureHeader: options.signatureHeader || void 0,
  ...
  try {
    let intentData = null;
    const { data: intentByProvider, error: intentProviderError } = await supabase.from("payment_intents").select("*").eq("provider_payment_id", String(paymentId)).maybeSingle();
    if (!intentProviderError && intentByProvider) {
      intentData = intentByProvider;
    } else {
      const { data: intentByBooking, error: intentBookingError } = await supabase.from("payment_intents").select("*").eq("booking_id", bookingId).order("created_at", { ascending: false }).limit(1).maybeSingle();
    ...
    const { error: intentUpdateError } = await supabase.from("payment_intents").update(intentUpdate).eq("id", intentData.id);
...
var worker = {
  async fetch(request, env, ctx) {
    return withSentry(request, env, ctx, async () => {
      ...
      try {
        const log = {
          info: /* @__PURE__ */ __name((msg, data) => console.log(`\u2139\uFE0F  ${msg}`, data || ""), "info"),
          error: /* @__PURE__ */ __name((msg, err) => console.error(`\u274C ${msg}`, err || ""), "error"),
          warn: /* @__PURE__ */ __name((msg, data) => console.warn(`\u26A0\uFE0F  ${msg}`, data || ""), "warn"),
          debug: /* @__PURE__ */ __name((msg, data) => console.debug(`\u{1F41B} ${msg}`, data || ""), "debug")
        };
...
// .wrangler/tmp/bundle-isCAeK/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-isCAeK/middleware-loader.entry.ts
