-- ============================================================================
-- FIX: get_available_cars - cast all enum types to TEXT
-- Date: 2026-01-08
-- ============================================================================
...
BEGIN
    ...
    SELECT
        c.id,
        c.owner_id,
        COALESCE(b.name, c.brand_text_backup)::TEXT as brand,
        COALESCE(m.name, c.model_text_backup)::TEXT as model,
        c.year,
        c.plate::TEXT,
    ...
    WHERE
        ...
        AND (NOT p_no_credit_card OR (
            c.payment_methods @> '["debit_card"]'::jsonb OR
            c.payment_methods @> '["cash"]'::jsonb OR
            c.payment_methods @> '["transfer"]'::jsonb OR
            c.payment_methods @> '["wallet"]'::jsonb
        ))
        AND (
