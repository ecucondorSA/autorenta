...
async function listNotifications(args) {
    ...
    const formatted = data.map((n) => `- [${n.is_read ? 'Leída' : 'No leída'}] ${n.title}
...
async function getNotificationStats(args) {
    const db = getSupabase();
    ...
    const readCount = readStats?.filter((n) => n.is_read).length || 0;
    const unreadCount = readStats?.filter((n) => !n.is_read).length || 0;
...
async function listUsers(args) {
    ...
    const formatted = data.map((u) => `- ${u.full_name || 'Sin nombre'}
   Email: ${u.email || 'N/A'}
   ID: ${u.id}
   Verificado: ${u.id_verified ? 'Sí' : 'No'}`).join('\n\n');
        ...
        try {
            ...
            switch (name) {
                ...
                case 'broadcast_notification':
                    result = await broadcastNotification(args);
                    break;
                case 'list_notifications':
                    result = await listNotifications(args);
                    break;
                case 'get_notification_stats':
                    result = await getNotificationStats(args);
                    break;
                case 'list_users':
                    result = await listUsers(args);
                    break;
                case 'delete_notification':
                    result = await deleteNotification(args);
                    break;
                default:
