...
export function registerTestGeneratorTool(
  gemini: GeminiClient,
  handlers: Map<string, (args: any) => Promise<any>>
) {
  handlers.set('gemini_generate_tests', async (args: unknown) => {
...
Provide complete, runnable test code as JSON.`;
    ...
    // Parse the response
    const result = gemini.parseJsonResponse<Omit<TestResult, 'file' | 'test_type'>>(response);
