...
export function registerUIAnalyzerTool(
  gemini: GeminiClient,
  handlers: Map<string, (args: any) => Promise<any>>
) {
  handlers.set('gemini_analyze_ui', async (args: unknown) => {
...
Be concise. Max 5 issues. Return ONLY the JSON object.`;
    ...
    try {
      result = gemini.parseJsonResponse<UIAnalysisResult>(response);
    } catch (e) {
    ...
    const filteredIssues = issues.filter(i =>
      i && i.category && allowedCategories.includes(i.category)
    );
