...
export class GeminiClient {
  ...
  private modelPro = 'gemini-3-pro-preview';  // Most powerful, reasoning-first
  private modelFlash = 'gemini-2.5-flash';    // Fast, good for vision
   ...
   */
  async generatePro(prompt: string, systemPrompt?: string): Promise<string> {
    try {
   ...
   */
  async generateFlash(prompt: string, systemPrompt?: string): Promise<string> {
    try {
      const response = await this.client.models.generateContent({
