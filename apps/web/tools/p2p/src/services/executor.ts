...
export class ExecutorService {
  ...
  private mpPage: MercadoPagoPage | null = null;
  ...
  private async executeOrder(order: P2POrder): Promise<void> {
    if (!this.mpPage) return;
    ...
    if (!destination) {
      ...
      await db.transitionStatus(
        ...
        'executor',
        undefined,
        'No payment destination (CVU/Alias)'
      );
      return;
    }
  ...
  private async handleQrVerification(order: P2POrder): Promise<void> {
    if (!this.mpPage) return;
