...
import type { DetectedOrder } from '../types/index.js';

const logger = createServiceLogger('detector');

export class DetectorService {
  private browser: BrowserManager;
  private binancePage: BinancePage | null = null;
  ...
  private async pollOnce(): Promise<void> {
    ...
    const pendingOrders = orders.filter(
      o => o.binanceStatus === 'pending' && o.orderType === 'buy'
    );
  ...
  private async processOrder(order: DetectedOrder): Promise<void> {
  ...
  private async extractPaymentDetails(order: DetectedOrder): Promise<void> {
    if (!this.binancePage || !order.href) return;
    ...
    try {
      ...
      if (details && (details.cvu || details.alias)) {
        ...
        await db.transitionStatus(
          order.orderNumber,
          'manual_review',
          'detector',
          undefined,
          'Could not extract payment details'
        );
      }
    } catch (error) {
