    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
          ...
          <div>
            ...
            <p class="text-sm text-text-secondary">
              ...
              @if (validPhotosCount() > 0) {
                <span class="text-emerald-600 ml-1"> ({{ validPhotosCount() }} verificadas) </span>
              }
            </p>
          </div>
        </div>
        ...
        <div class="flex items-center gap-2">
          ...
          @if (isGeneratingAI()) {
            <div
              class="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-cta-default to-cta-hover text-white rounded-lg text-xs font-bold"
            >
      ...
      @if (detectedVehicle() && showAutoDetect()) {
        <div class="detection-banner rounded-xl p-4 animate-fade-in">
          <div class="flex items-start gap-3">
            <div
              class="w-10 h-10 rounded-full bg-emerald-100/50 flex items-center justify-center flex-shrink-0 backdrop-blur-sm"
            >
              <svg
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        >
          <div class="flex flex-col items-center gap-2 p-4 text-center relative z-10">
            <div
              class="p-3 bg-surface-secondary rounded-full group-hover:scale-110 transition-transform shadow-sm"
            ...
            <div>
              ...
              <p class="text-xs text-text-muted mt-0.5 leading-tight">Tocá para agregar</p>
            </div>
          </div>
          <input
            type="file"
        ...
        @for (photo of photos(); track photo.id; let i = $index) {
          <div
            class="aspect-[4/3] relative rounded-xl overflow-hidden group bg-surface-secondary photo-card"
          >
            <!-- Image -->
            <img
            ...
            @if (photo.status === 'validating') {
              <div
                class="absolute inset-0 ai-scanning-overlay flex flex-col items-center justify-center gap-2"
              >
                <div class="ai-scan-line"></div>
                <div
                  class="relative z-10 bg-black/50 backdrop-blur-md px-3 py-1 rounded-full border border-white/10"
                >
                  <span class="text-white text-xs font-mono tracking-wider"
                    >ANALYZING {{ photo.progress }}%</span
                  >
                </div>
              </div>
            }
            ...
            @if (photo.status !== 'validating') {
              <div class="absolute top-2 left-2">
                @if (photo.status === 'valid') {
                  <div
                    class="flex items-center gap-1 px-2 py-1 bg-emerald-500 text-white rounded-full text-[10px] font-bold shadow-sm uppercase tracking-wider border border-white/20 backdrop-blur-md"
                  >
                  ...
                  <div
                    class="flex items-center gap-1 px-2 py-1 bg-amber-500 text-white rounded-full text-[10px] font-bold shadow-sm uppercase tracking-wider border border-white/20 backdrop-blur-md"
                  >
                    <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path
                        stroke-linecap="round"
            ...
            @if (photo.plates?.detected) {
              <div
                class="absolute top-2 right-2 px-2 py-1 bg-blue-500/90 text-white rounded-full text-[10px] font-bold shadow-sm flex items-center gap-1 uppercase tracking-wider border border-white/20 backdrop-blur-md"
              >
                <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path
                    stroke-linecap="round"
            ...
            @if (i === 0) {
              <div
                class="absolute bottom-2 left-2 px-2.5 py-1 bg-slate-900 rounded-md shadow-lg border border-white/20"
              >
            ...
            @if (photo.quality?.issues?.length) {
              <div class="absolute bottom-2 right-2 group/tooltip">
                <div
                  class="p-1.5 bg-amber-500 rounded-full cursor-help hover:scale-110 transition-transform shadow-lg border border-white/20"
                >
                  <svg
                ...
                <div
                  class="absolute bottom-full right-0 mb-2 w-48 p-3 bg-slate-900/95 text-white text-xs rounded-xl opacity-0 invisible group-hover/tooltip:opacity-100 group-hover/tooltip:visible transition-all shadow-xl z-10 border border-white/10 backdrop-blur-xl"
                >
                  <p class="font-bold mb-2 text-amber-400 uppercase tracking-wide text-[10px]">
                    Sugerencias AI
                  </p>
                  <ul class="space-y-1">
        ...
        @for (slot of emptySlots(); track slot) {
          <div
            class="aspect-[4/3] rounded-xl stencil-slot flex flex-col items-center justify-center p-3 group/stencil cursor-default"
          >
            <div class="text-center">
              <div
                class="w-12 h-12 mx-auto mb-3 rounded-full bg-white shadow-sm flex items-center justify-center group-hover/stencil:scale-110 transition-transform border border-border-default"
              >
                @switch (slot) {
                  @case ('front') {
                    <svg
                      class="w-6 h-6 stencil-icon"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                    >
              ...
              <span
                class="text-xs font-bold text-text-secondary uppercase tracking-wider block mb-1 group-hover/stencil:text-cta-default transition-colors"
                >{{ getPositionLabel(slot) }}</span
              >
              <p class="text-[10px] text-text-muted mt-0.5 leading-tight max-w-[120px] mx-auto">
                {{ getPositionHint(slot) }}
              </p>
            </div>
          </div>
        }
        ...
        <div class="flex items-start gap-3 p-4 bg-blue-50 border border-blue-100 rounded-xl">
          ...
          <div>
            ...
            <p class="text-xs text-blue-700 mt-0.5">
              Autos con 5+ fotos reciben hasta <strong>40% más consultas</strong>. Usá luz natural y
              mostrá el vehículo completo.
            </p>
          </div>
        </div>
      }
    ...
    `
      ...
      @keyframes pulseBorder {
        ...
        50% {
          ...
          background-color: rgba(0, 217, 95, 0.08);
        }
        100% {
          border-color: rgba(0, 217, 95, 0.3);
          background-color: rgba(0, 217, 95, 0.02);
...
export class PhotoUploadAIComponent {
  ...
  private async runQualityValidation(photo: PhotoWithAI): Promise<void> {
    ...
    try {
      const result = await this.photoQualityService.validatePhoto(
        photo.preview,
        'vehicle_exterior',
      );
