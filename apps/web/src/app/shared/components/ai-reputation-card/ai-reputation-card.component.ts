      ...
      <div class="p-4">
        ...
        @if (!hasAnalyzed() && !loading()) {
          <div class="text-center py-4">
            <div
              class="w-12 h-12 mx-auto mb-3 rounded-xl bg-cta-default/10 flex items-center justify-center"
            >
              <svg
        ...
        @if (loading()) {
          <div class="py-4 text-center">
            <div
              class="w-8 h-8 mx-auto mb-2 border-2 border-cta-default/20 border-t-cta-default rounded-full animate-spin"
            ></div>
        ...
        @if (error() && !loading()) {
          <div class="py-4 text-center">
            ...
            <button
              (click)="analyze()"
              class="text-sm text-cta-default hover:underline font-medium"
            >
              Reintentar
            </button>
          </div>
        ...
        @if (analysis() && !loading()) {
          <div class="space-y-4">
                ...
                <ul class="space-y-1.5">
                  @for (highlight of analysis()!.highlights; track highlight) {
                    <li class="flex items-start gap-2 text-sm text-text-primary">
                      <svg
                        class="w-4 h-4 text-success-default mt-0.5 flex-shrink-0"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                ...
                <ul class="space-y-1.5">
                  @for (area of analysis()!.improvementAreas; track area) {
                    <li class="flex items-start gap-2 text-sm text-text-secondary">
                      <svg
                        class="w-4 h-4 text-warning-default mt-0.5 flex-shrink-0"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
            ...
            <div class="flex items-center justify-between pt-3 border-t border-border-default">
              ...
              <button
                (click)="regenerate()"
                class="text-xs text-text-secondary hover:text-cta-default transition-colors"
              >
                Regenerar
              </button>
            </div>
