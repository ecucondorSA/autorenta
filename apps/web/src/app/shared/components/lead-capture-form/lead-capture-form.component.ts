...
@Component({
  ...
  template: `
    <div
      class="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-xl max-w-md mx-auto overflow-hidden relative"
    >
      ...
      @if (!isSubmitted()) {
        <div class="relative z-10">
          <div class="mb-6">
            <h3 class="text-2xl font-black text-white mb-2 tracking-tight text-balance">
              Comienza a ganar en dólares
            </h3>
            <p class="text-zinc-400 text-sm">
              Déjanos tus datos y un asesor te ayudará con el alta de tu vehículo.
            </p>
          </div>
          ...
          <form [formGroup]="leadForm" (ngSubmit)="onSubmit()" class="space-y-4">
            ...
            <div class="space-y-1">
              <label
                class="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest ml-1"
                >Nombre y Apellido</label
              >
              <div class="relative group">
                <ion-icon
                  name="person-outline"
                  class="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-ivory-400 transition-colors"
                ></ion-icon>
                <input
                  type="text"
            ...
            <div class="pt-2">
              <label
                class="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-3 ml-1"
                >¿Tenés auto propio?</label
              >
              <div class="flex gap-3">
                <button
                  ...
                  [class]="
                    leadForm.get('has_car')?.value === true
                ...
                <button
                  ...
                  [class]="
                    leadForm.get('has_car')?.value === false
                      ? 'bg-white text-black border-white'
                      : 'bg-zinc-950 text-zinc-400 border-zinc-800'
                  "
                  class="flex-1 py-3 px-4 rounded-xl border font-bold text-sm transition-all active:scale-95"
            ...
            <p class="text-[9px] text-center text-zinc-600 px-4">
              Al enviar, aceptas que AutoRenta te contacte para fines comerciales. Tus datos están
              protegidos por nuestra política de privacidad.
            </p>
          </form>
        </div>
      } @else {
        <div class="text-center py-12 px-4 animate-fade-in relative z-10">
          ...
          <p class="text-zinc-400 mb-8 leading-relaxed">
            Un experto de AutoRenta te contactará por WhatsApp para ayudarte a ganar dinero con tu
            auto.
          </p>
          <button
            (click)="resetForm()"
            class="px-6 py-2 rounded-full border border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-600 transition-all text-xs font-bold uppercase tracking-widest"
          >
