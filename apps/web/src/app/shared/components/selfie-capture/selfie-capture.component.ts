...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-50 flex flex-col bg-black text-white">
      <!-- Header -->
      <div class="absolute top-0 left-0 right-0 z-10 p-4 flex justify-between items-center bg-gradient-to-b from-black/80 to-transparent">
        <button (click)="cancelled.emit()" class="p-2 rounded-full bg-white/10 backdrop-blur hover:bg-white/20 transition-colors">
      ...
      <div class="flex-1 flex flex-col justify-center items-center relative overflow-hidden">
        ...
        <!-- Oval Overlay (Mask) -->
        <div class="absolute inset-0 pointer-events-none z-10 border-[50vw] border-black/50 rounded-[50%]" 
        ...
        <div class="absolute inset-0 z-10 pointer-events-none flex items-center justify-center">
           <svg class="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
             ...
             <!-- Guide Ring -->
             <ellipse cx="50" cy="45" rx="36" ry="41" fill="none" 
      ...
      <div class="bg-black p-8 pb-Safe z-20 flex flex-col items-center gap-6">
        ...
        @if (!recordedBlob()) {
            ...
            @if (recording()) {
               <svg class="absolute inset-0 w-full h-full -rotate-90 pointer-events-none">
                 <circle cx="50%" cy="50%" r="36" fill="none" stroke="red" stroke-width="4" 
                         stroke-dasharray="226" [attr.stroke-dashoffset]="226 - (226 * progress() / 100)" 
          ...
          <div class="flex w-full gap-4">
             <button (click)="retake()" class="flex-1 py-4 rounded-xl bg-white/10 font-bold hover:bg-white/20 transition-colors">
               Repetir
             </button>
             <button (click)="confirmUpload()" class="flex-1 py-4 rounded-xl bg-green-500 text-black font-bold hover:bg-green-400 transition-colors flex items-center justify-center gap-2">
               @if (uploading()) {
  ...
  `,
  styles: [`
    :host { display: block; position: fixed; inset: 0; z-index: 50; }
  `]
})
