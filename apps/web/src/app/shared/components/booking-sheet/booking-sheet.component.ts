  ...
  template: `
    @if (car) {
      <div class="fixed inset-0 z-50 flex items-end justify-center pointer-events-none">
        ...
        <div
          class="absolute inset-0 bg-black/40 backdrop-blur-sm transition-opacity duration-500 pointer-events-auto"
          (click)="dismiss()"
        ></div>
        ...
        >
          ...
          <div
            class="w-12 h-1 bg-white/30 rounded-full mx-auto mb-6 cursor-grab active:cursor-grabbing hover:bg-white/50 transition-colors"
          ></div>

          <div class="flex gap-4">
            <!-- Large Photo -->
            <div
              class="w-24 h-24 rounded-2xl overflow-hidden bg-surface-secondary border border-white/10 shadow-lg shrink-0 relative group"
            >
              <img
            ...
            <div class="flex-1">
              <h2 class="text-2xl font-black text-white font-satoshi tracking-tight">
                {{ car.title }}
              </h2>
              <div class="flex items-center gap-2 mt-1">
                @if (car.instantBooking) {
                  <span
                    class="text-brand-primary text-sm font-bold bg-brand-primary/10 px-2 py-0.5 rounded flex items-center gap-1"
                  >
                    <svg class="w-3 h-3 animate-pulse" viewBox="0 0 24 24" fill="currentColor">
                  ...
                  <span
                    class="text-brand-primary text-sm font-bold bg-brand-primary/10 px-2 py-0.5 rounded"
                    >Premium</span
                  >
                }
          ...
          <div class="grid grid-cols-3 gap-3 mt-6">
            <div
              class="bg-zinc-800/40 backdrop-blur-md rounded-2xl p-3 text-center border border-white/5"
            >
              <div class="text-zinc-500 text-[10px] uppercase font-bold tracking-wider mb-1">
                Caja
              </div>
            ...
            >
              ...
              <div class="text-white font-bold text-sm truncate capitalize">
                {{ car.fuelType || 'Nafta' }}
              </div>
            </div>
          </div>
          ...
          @if (car.ownerVerified === false) {
            <div class="mt-6 rounded-2xl bg-zinc-900/60 border border-white/10 p-4">
              ...
              <p class="text-zinc-300 text-sm mt-1">
                Este vehiculo es visible, pero no se puede reservar hasta que el propietario
                complete su verificacion de identidad.
              </p>
            </div>
          }
          ...
          <div class="mt-8 flex items-center justify-between gap-4 border-t border-white/5 pt-6">
            <div class="text-white">
              <div class="text-[10px] text-zinc-500 uppercase tracking-wider font-bold mb-1">
                Aporte diario
              </div>
