...
@Component({
  ...
  template: `
    <div class="verification-blocking-modal" [class.open]="isOpen()">
      ...
      >
        <div
          class="bg-white dark:bg-gray-700-pure dark:bg-anthracite rounded-lg shadow-xl p-6 max-w-md w-full"
        >
          <!-- Close Button -->
          <button
            (click)="close()"
            class="float-right text-charcoal-medium dark:text-pearl-light/70 hover:text-gray-900 dark:hover:text-ivory-luminous"
          ...
          <h2
            id="modal-title"
            class="text-xl font-bold text-gray-900 dark:text-ivory-luminous text-center mb-2"
          >
            {{ config.title }}
          </h2>
          ...
          <div class="flex gap-3 flex-col-reverse sm:flex-row">
            <button
              (click)="close()"
              class="flex-1 px-4 py-2 border border-pearl-gray dark:border-neutral-700 text-gray-900 dark:text-ivory-luminous font-medium rounded-lg hover:bg-ivory-100 dark:hover:bg-graphite-dark transition-colors"
            >
              Cancelar
            </button>
            <a
