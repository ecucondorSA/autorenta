...
import { VehicleDocumentsService, VehicleDocument } from '@core/services/verification/vehicle-documents.service';
      ...
      <div class="bg-surface-raised rounded-xl sm:rounded-2xl border border-border-default shadow-sm">
            ...
            <div class="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                ...
                <p class="text-xs text-text-secondary">
                  Cédula verde, VTV y seguro de tus autos
                </p>
              </div>
            </div>
            @if (totalMissing() > 0) {
              <span class="px-2 py-1 text-xs font-bold bg-warning-light/20 text-warning-strong rounded-full">
        ...
        <div class="divide-y divide-border-default">
          @for (car of cars(); track car.carId) {
            ...
            >
              <!-- Car thumbnail -->
              <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-lg overflow-hidden bg-surface-base flex-shrink-0">
                @if (car.carImage) {
                  <img [src]="car.carImage" [alt]="car.carTitle" class="w-full h-full object-cover">
                } @else {
              ...
              <div class="flex-grow min-w-0">
                <p class="text-sm font-medium text-text-primary truncate group-hover:text-primary-600 transition-colors">
                  {{ car.carTitle }}
                </p>
              ...
              <!-- Arrow -->
              <svg class="w-5 h-5 text-text-secondary group-hover:text-primary-600 transition-colors flex-shrink-0"
  ...
  private async loadCarsWithDocStatus(): Promise<void> {
    try {
      ...
      const carStatuses: CarDocStatus[] = userCars.map(car => {
        ...
        const vtvExpiringSoon = hasVtv && vtvExpiry && vtvExpiry > now && vtvExpiry <= thirtyDaysFromNow;
        const insuranceExpiringSoon = hasInsurance && insuranceExpiry && insuranceExpiry > now && insuranceExpiry <= thirtyDaysFromNow;
