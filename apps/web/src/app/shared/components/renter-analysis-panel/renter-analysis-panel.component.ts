      ...
      @if (loading()) {
          ...
          <div class="warnings-section">
            @for (warning of analysis()!.warnings; track warning.type) {
              <div class="warning-item" [class]="'severity-' + warning.severity">
                <app-icon
                  [name]="warning.severity === 'alert' ? 'alert-triangle' : warning.severity === 'warning' ? 'alert-circle' : 'info'"
                  [size]="18"
                />
        ...
        <div class="section profile-section">
          <div class="profile-header">
            ...
            <div class="profile-info">
              ...
              <p class="profile-member">Miembro desde {{ formatDate(analysis()!.profile.member_since) }}</p>
              @if (analysis()!.profile.years_as_member > 0) {
                <span class="member-badge">{{ analysis()!.profile.years_as_member }} {{ analysis()!.profile.years_as_member === 1 ? 'ano' : 'anos' }}</span>
              }
            </div>
          </div>
        ...
        <div class="section verification-section">
          ...
          <div class="verification-grid">
            <div class="verification-item" [class.verified]="analysis()!.verification.id_verified">
              <app-icon [name]="analysis()!.verification.id_verified ? 'check-circle' : 'circle'" [size]="20" />
              <span>Identidad</span>
            </div>
            <div class="verification-item" [class.verified]="analysis()!.verification.phone_verified">
              <app-icon [name]="analysis()!.verification.phone_verified ? 'check-circle' : 'circle'" [size]="20" />
              <span>Telefono</span>
            </div>
            <div class="verification-item" [class.verified]="analysis()!.verification.email_verified">
              <app-icon [name]="analysis()!.verification.email_verified ? 'check-circle' : 'circle'" [size]="20" />
              <span>Email</span>
            </div>
            <div class="verification-item" [class.verified]="analysis()!.verification.license_verified">
              <app-icon [name]="analysis()!.verification.license_verified ? 'check-circle' : 'circle'" [size]="20" />
              <span>Licencia</span>
            </div>
          </div>
        </div>
        ...
        <div class="section stats-section">
          ...
          <div class="stats-grid">
            ...
            <div class="stat-item">
              <span class="stat-value">{{ getRiskLevelEmoji(analysis()!.risk_profile.risk_level) }}</span>
              ...
              <div class="rating-stars">
                @for (star of [1,2,3,4,5]; track star) {
                  <app-icon
                    name="star"
                    [size]="20"
                    [cssClass]="star <= Math.round(analysis()!.reviews_summary.avg_rating) ? 'text-yellow-500 fill-yellow-500' : 'text-gray-300'"
                  />
                }
              </div>
              <span class="review-count">{{ analysis()!.reviews_summary.count }} {{ analysis()!.reviews_summary.count === 1 ? 'review' : 'reviews' }}</span>
            </div>
