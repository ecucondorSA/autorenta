...
@Component({
  ...
  template: `
    ...
    >
      @if (loading()) {
        <svg class="animate-spin -ml-1 mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  async makePublic(): Promise<void> {
    ...
    try {
      ...
      const response = await this.http
        .post<{ success: boolean; message: string; calendar_id: string }>(
      ...
      this.notifications.error(
        'Error',
        `No se pudo hacer público el calendario: ${errorMessage}`
      );
    } finally {
