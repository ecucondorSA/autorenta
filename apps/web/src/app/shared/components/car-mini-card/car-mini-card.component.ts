...
@Component({
  ...
  template: `
    <div
      ...
      <div class="relative h-full bg-zinc-900 border border-zinc-800 rounded-2xl p-3 flex gap-4 transition-all duration-300 hover:border-zinc-700 hover:bg-zinc-800/80"
        ...
        <div class="flex-1 flex flex-col justify-center min-w-0">
          <div class="flex justify-between items-start gap-2">
            <div class="min-w-0">
              <h3 class="font-bold text-white leading-tight font-sans truncate text-ellipsis text-sm">{{ car.title }}</h3>
              <div class="flex items-center gap-2 mt-1">
                <p class="text-[10px] text-zinc-400 truncate uppercase tracking-wide font-bold">{{ car.city }}</p>
                <!-- Rating Stars -->
                <div class="flex items-center gap-0.5 bg-zinc-950 px-1.5 py-0.5 rounded border border-white/5">
            ...
            <div class="flex items-center gap-1 shrink-0">
              @if (car.instantBooking) {
                <div class="bg-emerald-500/10 p-1 rounded-md" title="Reserva Instantánea">
                  <svg class="text-emerald-500 w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
                </div>
              }
            </div>
          </div>
          ...
          <div class="mt-3 flex items-center justify-between gap-2 border-t border-white/5 pt-2">
            <span class="text-sm font-black text-white tracking-tight shrink-0 flex items-baseline gap-1">
