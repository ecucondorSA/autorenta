...
@Component({
  ...
  template: `
    <div class="relative group">
      ...
      <div
        class="absolute -inset-0.5 bg-gradient-to-r from-yellow-500 via-amber-500 to-orange-500 rounded-2xl opacity-75 blur-sm group-hover:opacity-100 transition-opacity duration-500 animate-gradient-xy"
      ></div>
      ...
      <div class="relative bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl overflow-hidden">
        <!-- Top gradient line -->
        <div
          class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-yellow-500 via-amber-500 to-orange-500"
        ></div>
        ...
        <div class="p-5 text-white">
          ...
          <div class="text-center mb-5">
            <div class="flex items-center justify-center gap-2 mb-2">
              <div class="relative">
                <div
                  class="absolute inset-0 bg-yellow-500/30 rounded-full blur-md animate-pulse"
                ></div>
                <div
                  class="relative w-10 h-10 bg-gradient-to-br from-yellow-400 to-amber-500 rounded-full flex items-center justify-center shadow-lg"
                >
                  <svg
            ...
            <p class="text-xs text-gray-400 mt-1">
              Tu amigo recibe $10 de bienvenida. Vos ganás $15 cuando su auto tenga su primera renta.
            </p>
          </div>
          ...
          @if (loading()) {
            <div class="space-y-4">
              <div
                class="h-16 bg-gradient-to-r from-gray-800 via-gray-700 to-gray-800 rounded-xl animate-shimmer bg-[length:200%_100%]"
              ></div>
              <div class="flex gap-4 justify-center">
                <div
                  class="h-12 w-24 bg-gradient-to-r from-gray-800 via-gray-700 to-gray-800 rounded-lg animate-shimmer bg-[length:200%_100%]"
                ></div>
                <div
                  class="h-12 w-24 bg-gradient-to-r from-gray-800 via-gray-700 to-gray-800 rounded-lg animate-shimmer bg-[length:200%_100%]"
                ></div>
          ...
          @if (!loading() && referralCode()) {
            ...
            >
              ...
              <div
                class="absolute inset-0 bg-gradient-to-r from-yellow-500/0 via-yellow-500/5 to-yellow-500/0 opacity-0 group-hover/code:opacity-100 transition-opacity duration-300"
              ></div>

              <div class="relative flex items-center justify-center gap-3">
                <span
                  class="font-mono text-2xl tracking-[0.3em] font-bold bg-gradient-to-r from-yellow-300 via-amber-400 to-yellow-300 bg-clip-text text-transparent"
                >
                  {{ referralCode()!.code }}
                </span>
                <div
                  class="w-8 h-8 bg-gray-700/50 rounded-lg flex items-center justify-center group-hover/code:bg-yellow-500/20 transition-colors"
                >
                  <svg
            ...
            @if (stats()) {
              <div class="flex items-center justify-center gap-4 mb-5">
                <div
                  class="flex-1 bg-gray-800/50 rounded-xl p-3 text-center border border-gray-700/50"
                >
                ...
                <div
                  class="flex-1 bg-gradient-to-br from-emerald-900/30 to-emerald-800/20 rounded-xl p-3 text-center border border-emerald-700/30"
                >
                  <div class="text-2xl font-bold text-emerald-400">
                    <span class="text-sm">$</span>{{ totalEarned() | number: '1.0-0' }}
                  </div>
                  <div class="text-xs text-emerald-400/70 uppercase tracking-wider font-medium">
                    Ganado
                  </div>
                </div>
              </div>
            ...
            @if (shareableLink()) {
              ...
              >
                ...
                <div
                  class="absolute inset-0 bg-gradient-to-r from-white/0 via-white/30 to-white/0 -translate-x-full group-hover/btn:translate-x-full transition-transform duration-700"
                ></div>
              </button>
            }
          }
          ...
          @if (!loading() && !referralCode()) {
            <div class="text-center py-6">
              <div class="relative w-16 h-16 mx-auto mb-4">
                <div
                  class="absolute inset-0 bg-gradient-to-br from-yellow-500/20 to-amber-500/20 rounded-full animate-pulse"
                ></div>
                <div
                  class="relative w-full h-full bg-gray-800 rounded-full flex items-center justify-center border border-gray-700"
                >
                  <svg
              ...
              >
                @if (generating()) {
                  <span class="flex items-center gap-2">
                    <div
                      class="w-4 h-4 border-2 border-gray-500 border-t-gray-900 rounded-full animate-spin"
                    ></div>
                    Generando...
                  </span>
                } @else {
