...
describe('Database Indexes', () => {
  ...
  describe('Bookings Table Indexes', () => {
    it('debe tener índice idx_bookings_car_dates para búsquedas por auto y fechas', async () => {
      const mockIndexData = {
        data: [{
          ...
          indexdef: 'CREATE INDEX idx_bookings_car_dates ON bookings USING btree (car_id, start_at, end_at)'
        }],
        error: null
      };
    ...
    it('idx_bookings_car_dates debe usar BTREE para range queries', () => {
      const indexDef = 'CREATE INDEX idx_bookings_car_dates ON bookings USING btree (car_id, start_at, end_at)';
  ...
  describe('Critical Indexes Verification', () => {
    it('debe tener todos los 5 índices críticos definidos', () => {
      const criticalIndexes = [
        { name: 'idx_bookings_car_dates', table: 'bookings', columns: ['car_id', 'start_at', 'end_at'] },
