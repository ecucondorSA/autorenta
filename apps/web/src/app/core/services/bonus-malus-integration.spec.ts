...
describe('Bonus-Malus Integration Tests', () => {
  ...
  describe('Driver Profile RPCs', () => {
    it(
      'should initialize and retrieve driver profile',
      async () => {
        if (!isIntegrationTest) return;
      ...
      30000
    ); // 30s timeout for integration test

    it(
      'should update driver class after clean booking',
      async () => {
        if (!isIntegrationTest) return;
    ...
    it(
      'should worsen class after claim with fault',
      async () => {
        if (!isIntegrationTest) return;
  ...
  describe('Autorentar Credit RPCs', () => {
    it(
      'should issue autorentar credit',
      async () => {
        if (!isIntegrationTest) return;
        ...
        const { data: creditInfo } = await supabaseClient.rpc(
    ...
    it(
      'should consume autorentar credit for claim',
      async () => {
        if (!isIntegrationTest) return;
    ...
    it(
      'should handle waterfall payment (credit + wallet)',
      async () => {
        if (!isIntegrationTest) return;
  ...
  describe('Bonus Protector RPCs', () => {
    it(
      'should purchase and retrieve bonus protector',
      async () => {
        if (!isIntegrationTest) return;
    ...
    it(
      'should prevent purchasing second protector while one is active',
      async () => {
        if (!isIntegrationTest) return;
  ...
  describe('Telemetry RPCs', () => {
    it(
      'should record telemetry and calculate driver score',
      async () => {
        if (!isIntegrationTest) return;
    ...
    it(
      'should calculate telemetry summary correctly',
      async () => {
        if (!isIntegrationTest) return;
  ...
  describe('Class Benefits RPC', () => {
    it(
      'should calculate class benefits correctly',
      async () => {
        if (!isIntegrationTest) return;
        ...
        expect(data[0].next_better_fee_multiplier).toBeLessThan(
          data[0].current_fee_multiplier
        );
      },
      30000
    );
  ...
  describe('Credit Renewal and Breakage RPCs', () => {
    it(
      'should extend credit for users with good history',
      async () => {
        if (!isIntegrationTest) return;
