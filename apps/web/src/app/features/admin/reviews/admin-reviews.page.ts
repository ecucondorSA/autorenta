...
@Component({
  ...
  template: `
    <div class="max-w-7xl mx-auto py-8 px-4">
      ...
      <div class="mb-6">
        ...
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-bold text-text-primary">Moderación de Reseñas</h1>
            <p class="text-text-secondary mt-1">Gestiona reseñas reportadas por usuarios</p>
          </div>
          <div class="text-right">
            <p class="text-sm text-text-secondary">Reseñas pendientes</p>
      ...
      @if (loading()) {
        <div class="text-center py-12">
          ...
          ></div>
          <p class="mt-4 text-text-secondary">Cargando reseñas...</p>
        </div>
      }

      <!-- Error State -->
      @if (error() && !loading()) {
        <div class="bg-error-bg border border-error-border rounded-lg p-6 text-center">
      ...
      @if (!loading() && !error()) {
        <div class="space-y-4">
          ...
          @if (filteredReviews().length === 0) {
            <div class="bg-surface-raised rounded-lg border border-border-default p-12 text-center">
              <svg
              ...
              <h3 class="mt-2 text-sm font-medium text-text-primary">No hay reseñas reportadas</h3>
          ...
          <!-- Review Cards -->
          @for (review of filteredReviews(); track review) {
            <div
              class="bg-surface-raised rounded-lg border border-border-default p-6 shadow-sm hover:shadow-md transition-shadow"
            >
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                  ...
                  <div class="grid grid-cols-3 gap-2 mb-4">
                    @for (category of getCategoryRatings(review); track category) {
                  ...
                  <!-- Public Comment -->
                  <div class="bg-surface-base rounded-lg p-3 mb-4">
                    <p class="text-sm font-medium text-text-primary mb-1">Comentario Público:</p>
                    <p class="text-sm text-text-primary">
                      {{ review.comment_public || '(Sin comentario público)' }}
                    </p>
                  </div>
                  <!-- Flag Reason -->
                  <div class="bg-error-bg rounded-lg p-3 mb-4">
                  ...
                  @if (review.moderation_notes) {
                    <div class="bg-cta-default/10 rounded-lg p-3 mb-4">
                      <p class="text-sm font-medium text-cta-default mb-1">Notas de Moderación:</p>
    ...
    @if (showModal()) {
      ...
      >
        <div
          class="bg-surface-raised rounded-lg shadow-xl max-w-md w-full p-6"
        ...
        >
          ...
          <div class="flex gap-2">
            <button
              (click)="closeModal()"
              class="flex-1 px-4 py-2 bg-surface-hover text-text-primary rounded-lg hover:bg-surface-pressed transition-colors text-sm font-medium"
            >
              Cancelar
            </button>
            <button
