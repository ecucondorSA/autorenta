...
@Component({
	...
	template: `
		<!-- Minimal Header -->
		<header class="sticky top-0 z-50 bg-white border-b border-gray-100 pt-[env(safe-area-inset-top)]">
		...
		<main class="min-h-[calc(100dvh-56px)] bg-gray-50 pb-safe">
			<div class="max-w-lg mx-auto px-4 py-6">
				...
				<div class="mb-8">
					...
					<p class="text-xs text-gray-500 mt-1 text-right">{{ progressPercentage() }}% completado</p>
				</div>
					...
					<div class="p-5">
						<div class="flex items-start gap-4">
							...
							<div class="flex-1 min-w-0">
								...
								<p class="text-sm text-gray-600 mt-1 truncate">{{ userEmail() || 'No registrado' }}</p>
								...
								@if (!isEmailVerified()) {
									...
									<button
										...
										[disabled]="emailVerificationService.loading() || !emailVerificationService.status().canResend"
										...
										<div class="flex gap-2">
											...
											<div class="relative">
												...
												@if (showCountryDropdown()) {
													<div class="absolute top-full left-0 mt-1 w-56 bg-white border border-gray-200 rounded-xl shadow-lg z-50 max-h-60 overflow-y-auto">
														@for (country of countries; track country.code) {
															...
															>
																...
																<span class="flex-1 text-left text-sm text-gray-700">{{ country.name }}</span>
											...
											<div class="space-y-2">
												...
												<div class="flex items-center justify-between">
													...
													<button
														...
														[disabled]="!phoneVerificationService.status().canResend || phoneVerificationService.loading()"
...
export class ContactVerificationPage implements OnInit, OnDestroy {
	...
	readonly userPhone = computed(
		() => this.profileStore.profile()?.phone || "",
	);
	...
	readonly isEmailVerified = computed(
		() => this.emailVerificationService.status().isVerified,
	);

	readonly isPhoneVerified = computed(
		() => this.phoneVerificationService.status().isVerified,
	);

	readonly phoneOtpSent = computed(
		() => this.phoneVerificationService.status().otpSent,
	);
