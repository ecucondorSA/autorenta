...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, CarCardComponent, CarsMapComponent, CarCarouselComponent, BookingSheetComponent],
  providers: [BrowseStore],
  templateUrl: './browse-cars.page.html',
  styles: [`
    :host { display: block; height: 100vh; width: 100vw; overflow: hidden; position: relative; background: black; }
    .split-layout { display: flex; height: 100%; width: 100%; }
    .list-panel { width: 100%; max-width: 450px; overflow-y: auto; background: var(--surface-base); z-index: 10; border-right: 1px solid var(--border-default); }
    .map-panel { flex: 1; position: relative; height: 100%; }
    @media (max-width: 768px) {
      .split-layout { flex-direction: column-reverse; }
      .list-panel { height: 40%; width: 100%; max-width: none; border-right: none; border-top: 1px solid var(--border-default); box-shadow: 0 -4px 20px rgba(0,0,0,0.5); border-radius: 24px 24px 0 0; }
      .map-panel { height: 60%; }
    }
    .scrollbar-premium::-webkit-scrollbar { width: 6px; }
    .scrollbar-premium::-webkit-scrollbar-track { background: transparent; }
    .scrollbar-premium::-webkit-scrollbar-thumb { background: rgba(57, 255, 20, 0.2); border-radius: 10px; }
    .scrollbar-premium::-webkit-scrollbar-thumb:hover { background: rgba(57, 255, 20, 0.5); }
  `]
})
export class BrowseCarsPage {
  ...
  readonly mapLocations = computed<CarMapLocation[]>(() => {
    ...
    return list.map(car => ({
      ...
      photoUrl: car.photos?.[0]?.url || car.car_photos?.[0]?.url || '/assets/images/car-placeholder.svg',
  ...
  async loadCars() {
    ...
    try {
      ...
      const results = await this.carsService.getAvailableCarsWithDistance(
        searchFrom,
        searchTo,
