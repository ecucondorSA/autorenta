...
export class BrowseStore {
    // State Signals
    readonly cars = signal<Car[]>([{ id: 'test-uuid', brand_text_backup: 'TEST', model_text_backup: 'NEON CAR', location_lat: -34.9011, location_lng: -56.1645, price_per_day: 100, currency: 'USD', status: 'active' } as any]);
    ...
    // Computed
    readonly activeCar = computed(() =>
        this.cars().find(c => c.id === this.activeCarId()) || null
    );
    ...
    setCars(cars: Car[]) {
        ...
        const testCar = { id: 'test-uuid', brand_text_backup: 'TEST', model_text_backup: 'CAR', location_lat: -34.9011, location_lng: -56.1645, price_per_day: 100, currency: 'USD', status: 'active' } as any; this.cars.set([testCar, ...cars]);
