...
@Component({
  ...
  template: `
    <div class="space-y-6">
      ...
      <div
        class="bg-surface-raised border border-border-default rounded-2xl overflow-hidden shadow-lg"
      >
        <!-- Photo placeholder -->
        <div
          class="aspect-video bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-700 dark:to-slate-800 relative"
        >
          ...
          <!-- Price badge -->
          <div
            class="absolute top-4 right-4 px-4 py-2 bg-black/70 backdrop-blur-sm text-white rounded-lg"
          >
          ...
          @if (data().isDynamicPricing) {
            <div
              class="absolute top-4 left-4 px-3 py-1 bg-emerald-500 text-white text-xs font-bold rounded-full"
            >
              PRECIO DINÁMICO
            </div>
          }
        ...
        <!-- Details -->
        <div class="p-5">
          <h3 class="text-xl font-bold text-text-primary">{{ data().brand }} {{ data().model }}</h3>
          <p class="text-text-secondary mt-1">
            {{ data().year }} • {{ formatMileage(data().mileage) }} km
          </p>
        ...
        <ul class="space-y-3">
          @for (item of checklistItems(); track item.label) {
            <li class="flex items-center gap-3">
              @if (item.valid) {
                <div
                  class="w-6 h-6 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center"
                >
                  <svg
      ...
      >
        ...
        <div>
          ...
          <p class="text-xs text-blue-700 dark:text-blue-400 mt-1">
            Aplicamos las configuraciones más populares: kilometraje ilimitado, combustible lleno a
            lleno, depósito 7% del valor del auto (preautorización) o 0% con Wallet AutoRentar.
      ...
      <!-- Earnings reminder -->
      <div
        class="text-center p-4 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-xl"
      >
...
export class SummaryQuestionComponent {
    ...
    return [
      {
        field: 'vehicle',
        label: 'Vehículo',
        value: `${d.brand} ${d.model} ${d.year}`,
        valid: true,
      },
      {
        field: 'photos',
        label: 'Fotos',
        value: `${d.photosCount} fotos`,
        valid: d.photosCount >= 3,
      },
      {
        field: 'mileage',
        label: 'Kilometraje',
        value: `${this.formatMileage(d.mileage)} km`,
        valid: d.mileage > 0,
      },
  ...
  readonly monthlyEarnings = computed(() => {
    ...
    return Math.round(price * 0.7 * 3.5 * 4);
  });
