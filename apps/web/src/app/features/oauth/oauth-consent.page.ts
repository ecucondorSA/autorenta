      ...
      <div class="w-full max-w-md">
        ...
        @if (error()) {
          <div class="bg-error-bg border-2 border-error-border rounded-xl p-6 text-center">
            <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-error-bg flex items-center justify-center">
              <svg class="w-8 h-8 text-error-strong" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @if (!loading() && !error() && authDetails()) {
          <div class="bg-surface-card border border-border-default rounded-xl shadow-lg overflow-hidden">
            <!-- Header -->
            ...
            <div class="p-6 space-y-6">
              ...
              <div class="text-center">
                <div class="w-16 h-16 mx-auto mb-3 rounded-full bg-cta-default/10 flex items-center justify-center">
                  <svg class="w-8 h-8 text-cta-default" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                  </svg>
                </div>
                <p class="text-text-secondary">
                  <span class="font-semibold text-text-primary">{{ authDetails()?.client?.name || 'Una aplicación' }}</span>
              ...
              @if (authDetails()?.scopes?.length) {
                <div class="bg-surface-base rounded-lg p-4">
                  <h3 class="text-sm font-medium text-text-secondary mb-3">Esta aplicación podrá:</h3>
                  <ul class="space-y-2">
                    @for (scope of authDetails()?.scopes; track scope) {
                      <li class="flex items-start gap-3">
                        <svg class="w-5 h-5 text-success-strong flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                        </svg>
                        <span class="text-sm text-text-primary">{{ getScopeDescription(scope) }}</span>
                      </li>
                    }
                  </ul>
                </div>
              ...
              <div class="bg-warning-bg/50 border border-warning-border rounded-lg p-3">
                <p class="text-xs text-warning-text">
                  Al autorizar, esta aplicación podrá acceder a la información indicada. Podés revocar el acceso en cualquier momento desde tu perfil.
                </p>
              </div>
            </div>
...
export class OAuthConsentPage implements OnInit {
  ...
  async approve(): Promise<void> {
    ...
    try {
      ...
      this.error.set(
        err instanceof Error ? err.message : 'Error al procesar la autorización.'
      );
