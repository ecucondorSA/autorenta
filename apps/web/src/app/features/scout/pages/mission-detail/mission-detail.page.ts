  ...
  template: `
    <div class="min-h-screen bg-gray-900 text-white">
      ...
      <div class="px-4 mt-2">
        <div class="bg-gray-800 rounded-3xl overflow-hidden border border-gray-700">
          <div class="h-48 bg-gray-700 relative">
            <img
              [src]="mission()?.cars?.photos?.[0]"
              class="w-full h-full object-cover"
              [alt]="(mission()?.cars?.brand || '') + ' ' + (mission()?.cars?.model || '')"
            />
            <div
              class="absolute top-4 right-4 bg-green-500 text-white px-3 py-1 rounded-full font-black text-sm shadow-lg"
            >
              RECOMPENSA: $150
            </div>
          </div>
            ...
            <div class="flex justify-between items-end">
              <div>
                ...
                <p class="text-gray-400">
                  {{ mission()?.cars?.color }} • {{ mission()?.cars?.year }}
                </p>
              </div>
              <div class="text-right">
                ...
                <p class="text-xl font-mono font-bold text-indigo-400">
                  {{ mission()?.cars?.license_plate | slice: 0 : 3 }} XXX
                </p>
              </div>
            </div>
      ...
      <div class="p-6 space-y-4">
        <h3 class="font-bold text-indigo-400 uppercase text-xs tracking-widest">
          Instrucciones de Seguridad
        </h3>
        <div class="space-y-3">
          <div class="flex gap-3 items-start">
            <div
              class="w-6 h-6 bg-indigo-500/20 text-indigo-400 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold"
            >
              1
            </div>
            <p class="text-sm text-gray-300">
              Mantén al menos 10 metros de distancia del vehículo.
            </p>
          </div>
          <div class="flex gap-3 items-start">
            <div
            ...
            <p class="text-sm text-gray-300">
              Asegúrate de que la **patente sea legible** en la foto.
            </p>
          </div>
          <div class="flex gap-3 items-start">
            <div
              class="w-6 h-6 bg-red-500/20 text-red-400 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold"
            >
              !
            </div>
            <p class="text-sm text-red-200 font-bold">
              NO interactúes con el conductor bajo ninguna circunstancia.
            </p>
          </div>
        </div>
      </div>
      ...
      <!-- ACTION BUTTON -->
      <div
        class="fixed bottom-0 left-0 w-full p-6 bg-gradient-to-t from-gray-900 via-gray-900 to-transparent"
      >
        <input
          type="file"
          #fileInput
...
export class MissionDetailPage implements OnInit {
  ...
  async onFileSelected(event: Event) {
    ...
    try {
      ...
      alert(
        '✅ Foto enviada correctamente. La IA está verificando la patente. Te avisaremos si la recompensa es aprobada.',
      );
