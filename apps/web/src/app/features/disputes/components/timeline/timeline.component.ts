      ...
      <div class="relative pl-6 border-l-2 border-border-default space-y-8">
        @for (event of events; track event.id) {
          <div class="relative">
            <!-- Icon -->
            <div
              class="absolute -left-[33px] top-0 h-4 w-4 rounded-full border-2 border-white"
              [ngClass]="{
            ...
            <div class="flex flex-col">
              ...
              @if (event.from_status && event.to_status) {
                <div class="text-xs text-text-muted mt-1">
                  Estado: <span class="font-medium">{{ event.from_status }}</span> →
                  <span class="font-medium text-text-primary">{{ event.to_status }}</span>
                </div>
              }
            </div>
          </div>
