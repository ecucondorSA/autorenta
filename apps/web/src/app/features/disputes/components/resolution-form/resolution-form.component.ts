  ...
  template: `
    <div class="bg-surface-raised border border-border-default rounded-2xl p-6 shadow-sm">
      ...
      <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-4">
        <!-- Favor -->
        <div>
          <label class="block text-sm font-medium text-text-secondary mb-2">Resolución a favor de:</label>
          <div class="grid grid-cols-3 gap-2">
            <button type="button" 
              ...
              class="px-4 py-2 border rounded-xl text-sm transition-colors">Arrendatario</button>
            <button type="button" 
              (click)="setFavor('owner')"
              [class.bg-blue-600]="form.value.resolutionFavor === 'owner'"
              [class.text-white]="form.value.resolutionFavor === 'owner'"
              class="px-4 py-2 border rounded-xl text-sm transition-colors">Propietario</button>
            <button type="button" 
              (click)="setFavor('none')"
              [class.bg-blue-600]="form.value.resolutionFavor === 'none'"
              [class.text-white]="form.value.resolutionFavor === 'none'"
              class="px-4 py-2 border rounded-xl text-sm transition-colors">Ninguno</button>
          </div>
        </div>
        ...
        @if (form.value.resolutionFavor === 'owner') {
          <div class="animate-in fade-in slide-in-from-top-2">
            <label class="block text-sm font-medium text-text-secondary mb-1">Monto de penalidad (USD):</label>
            <div class="relative">
            ...
            <p class="text-xs text-text-muted mt-1">Máximo disponible: {{ (maxDepositCents / 100) | currency:'USD' }}</p>
          </div>
        }

        <!-- Notas Públicas -->
        <div>
          <label class="block text-sm font-medium text-text-secondary mb-1">Explicación para las partes:</label>
          <textarea 
            ...
            placeholder="Describe el motivo de la decisión..."></textarea>
        </div>

        <!-- Notas Internas -->
        <div>
          <label class="block text-sm font-medium text-text-secondary mb-1">Notas internas (Solo Admin):</label>
          <textarea 
            formControlName="internalNotes"
            rows="2"
            class="w-full px-4 py-2 bg-surface-secondary border border-border-default rounded-xl outline-none"
            placeholder="Detalles técnicos o contables..."></textarea>
        </div>
