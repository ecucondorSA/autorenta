...
@Component({
  ...
  imports: [CommonModule, RouterLink, DisputeResolutionFormComponent, DisputeTimelineComponent],
  template: `
          ...
          <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-8 space-y-8">
              ...
              <section class="card-premium p-6">
                ...
                <div
                  class="inline-block px-2 py-1 bg-surface-secondary rounded-lg text-xs font-mono text-text-secondary mb-4"
                >
                  Motivo: {{ d()?.kind | uppercase }}
                </div>
                <p
                  class="text-text-secondary leading-relaxed whitespace-pre-wrap italic border-l-4 border-border-default pl-4"
                >
                  "{{ d()?.description }}"
                </p>
              </section>
              ...
              <section class="space-y-4">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                  @for (ev of evidence(); track ev.id) {
                    <div
                      class="group relative aspect-square bg-surface-raised rounded-2xl border border-border-default overflow-hidden shadow-sm hover:shadow-md transition-shadow"
                    >
                      <img
                        [src]="ev.path || ev.url"
                    ...
                    <div
                      class="col-span-full py-12 border-2 border-dashed border-border-default rounded-2xl flex flex-col items-center justify-center text-text-muted"
                    >
                      <svg
            ...
            <div class="lg:col-span-4 space-y-6">
              ...
              <section
                class="bg-indigo-900 text-white rounded-2xl p-6 shadow-lg overflow-hidden relative"
              >
                <div class="relative z-10">
                  <h3 class="text-xs uppercase tracking-widest font-bold opacity-60 mb-4">
                    Garantía del Contrato
                  </h3>
                  <div class="text-3xl font-black mb-1">
                    {{ (d()?.booking?.deposit_amount_cents || 0) / 100 | currency: 'USD' }}
                  </div>
                  ...
                  <div class="flex items-center gap-3">
                    <div
                      class="h-10 w-10 rounded-full bg-white/10 flex items-center justify-center font-bold text-sm"
                    >
                ...
                <!-- Decoración -->
                <div
                  class="absolute top-0 right-0 -mr-8 -mt-8 w-32 h-32 bg-white/5 rounded-full blur-3xl"
                ></div>
              </section>
                  ...
                  <div class="space-y-3">
                    ...
                    <div class="flex justify-between text-sm">
                      ...
                      <span class="font-bold text-red-600">{{
                        (d()?.penalty_amount_cents || 0) / 100 | currency: 'USD'
                      }}</span>
                    </div>
                    <div
                      class="pt-3 border-t border-border-default text-xs text-text-secondary italic"
                    >
                      "{{ d()?.internal_notes }}"
                    </div>
                  </div>
