...
@Component({
  ...
  template: `
    <div class="w-full py-6 px-4 bg-surface-raised rounded-2xl border border-border-default shadow-sm mb-6">
      <!-- Mobile View: Simple Progress Bar -->
      <div class="md:hidden">
        <div class="flex justify-between items-center mb-2">
          <span class="text-sm font-bold text-text-primary">{{ currentStepLabel() }}</span>
          <span class="text-xs text-text-muted">Paso {{ activeStepIndex() + 1 }} de {{ steps().length }}</span>
        </div>
        ...
        @for (step of steps(); track step.id; let i = $index) {
          <div class="relative z-10 flex flex-col items-center w-1/5 group">
            ...
            <div 
              ...
              [ngClass]="{
                'bg-cta-default border-cta-default text-white shadow-lg shadow-cta-default/20': step.status === 'completed' || step.status === 'current',
            ...
            <div class="mt-3 text-center">
              ...
              <p class="text-[10px] text-text-muted leading-tight mt-1 opacity-0 group-hover:opacity-100 transition-opacity max-w-[80px] mx-auto">
                {{ step.description }}
              </p>
            </div>
          </div>
...
export class BookingTimelineComponent {
  ...
  steps = computed(() => {
    ...
    const isPaid = s === 'confirmed' || s === 'in_progress' || s === 'completed' || s === 'returned';
