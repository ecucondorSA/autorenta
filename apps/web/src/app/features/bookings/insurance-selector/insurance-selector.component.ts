    ...
    <ion-card class="insurance-card">
      ...
      <ion-card-content>
        ...
        <div class="included-coverage">
          ...
          <div class="premium-info">
            <div class="premium-row" *ngIf="!hasOwnerInsurance">
              ...
              <span class="value">{{ dailyPremium | currency:'ARS':'symbol-narrow':'1.0-0' }}/día</span>
            </div>
            <div class="premium-row total" *ngIf="!hasOwnerInsurance">
              <span class="label">Total {{ rentalDays }} días:</span>
              <span class="value highlight">{{ totalBasePremium | currency:'ARS':'symbol-narrow':'1.0-0' }}</span>
            </div>
          ...
          <div class="deposit-info">
            ...
            <div class="deposit-text">
              ...
              <small>Este monto se congela en tu tarjeta durante el alquiler. Se devuelve automáticamente si no hay daños.</small>
            </div>
          </div>
        </div>
          ...
          <div class="addon-item" *ngFor="let addon of availableAddons">
            ...
            <div class="addon-content" (click)="toggleAddon(addon)">
              <div class="addon-header">
                <h4>{{ addon.name }}</h4>
                <span class="addon-price">+{{ addon.daily_cost | currency:'ARS':'symbol-narrow':'1.0-0' }}/día</span>
              </div>
              <p class="addon-description">{{ addon.description }}</p>
              <div class="addon-total" *ngIf="isAddonSelected(addon.id)">
                <ion-icon name="calculator" color="primary"></ion-icon>
                <span>Total: {{ (addon.daily_cost * rentalDays) | currency:'ARS':'symbol-narrow':'1.0-0' }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <ion-note color="medium" class="important-note">
          ...
          <div>
            <strong>En caso de accidente:</strong> Contacta inmediatamente al <strong>0800-AUTORENTAR</strong> (24/7).
            La franquicia se descuenta del depósito solo si hay daños.
          </div>
        </ion-note>
