...
@Component({
  ...
  template: `
    ...
    >
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        >
          <div class="flex flex-col items-center text-center space-y-2">
            <svg
              class="w-8 h-8 text-primary-600 dark:text-accent-petrol"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
      ...
      >
        <p
          class="text-xs text-charcoal-medium dark:text-pearl-light/80"
          *ngIf="selectedMode === 'card'"
        >
          <strong>Con tarjeta:</strong> Autorizamos un monto en tu tarjeta (hold). Si todo está OK,
          se libera completamente. Si hay consumos o daños, capturamos solo lo necesario (hasta tu
          franquicia).
        </p>
        <p
          class="text-xs text-charcoal-medium dark:text-pearl-light/80"
          *ngIf="selectedMode === 'wallet'"
        >
          <strong>Sin tarjeta:</strong> Depositás un Crédito de Seguridad (NO reembolsable) que
          queda como saldo no retirable en tu wallet y se usa primero para gastos/daños. Ideal si no
