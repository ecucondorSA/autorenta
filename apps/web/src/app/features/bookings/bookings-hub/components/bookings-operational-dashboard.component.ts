  ...
  template: `
    <div class="animate-in fade-in zoom-in duration-500 space-y-6">
      ...
      <div class="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm relative overflow-hidden">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="bg-slate-50 p-3 rounded-xl border border-slate-100">
            <span class="block text-[10px] uppercase font-bold text-slate-400 mb-1">Verificación</span>
          ...
          <div class="bg-slate-50 p-3 rounded-xl border border-slate-100">
            <span class="block text-[10px] uppercase font-bold text-slate-400 mb-1">Reputación</span>
            <div class="flex items-center gap-1.5 text-slate-700 font-bold text-xs">
      ...
      <div class="relative group cursor-pointer" [routerLink]="role() === 'owner' ? '/cars/publish' : '/cars/list'">
        ...
        <div class="p-6 min-h-[140px] flex flex-col items-center justify-center text-center relative z-10">
          <div class="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform duration-300">
            <ion-icon [name]="role() === 'owner' ? 'add-outline' : 'navigate-outline'" class="text-2xl text-slate-400"></ion-icon>
          </div>
          ...
          <p class="text-xs text-slate-400 max-w-[200px] mb-4">
            {{ role() === 'owner' ? 'Listo para sincronizar nuevo vehículo.' : 'Configuración de viaje lista para asignar.' }}
          </p>

          <span class="text-xs font-bold text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-lg group-hover:bg-indigo-100 transition-colors">
      ...
      @if (historyBookings().length > 0) {
        <div class="pt-4 border-t border-slate-100">
          <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 px-1">Historial Operativo</h3>
          <app-bookings-list
