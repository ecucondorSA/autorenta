-- =============================================
-- EXPAND PROFILES - Migración Incremental
-- Fecha: 2025-10-16
...
-- Rol del usuario (compatible con valores existentes)
DO $$ BEGIN
  CREATE TYPE user_role AS ENUM ('locador', 'locatario', 'ambos');
EXCEPTION
...
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS gov_id_type TEXT,        -- DNI/CI/Pasaporte
...
CREATE TABLE IF NOT EXISTS public.user_documents (
  ...
  storage_path TEXT NOT NULL,             -- ej: documents/{userId}/{uuid}.pdf
...
CREATE TABLE IF NOT EXISTS public.profile_audit (
  ...
  changes JSONB NOT NULL,                 -- diff {before, after} o par clave/valor
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- 6. ACTUALIZACIÓN DEL TRIGGER DE NUEVO USUARIO
-- =============================================

-- Actualizar función handle_new_user para incluir nuevos campos
...
BEGIN
  ...
  VALUES (
    ...
    'locatario',
    false,
    COALESCE(NEW.raw_user_meta_data->>'timezone', 'America/Montevideo'),
    COALESCE(NEW.raw_user_meta_data->>'locale', 'es-AR'),
    'not_started',
    'incomplete',
