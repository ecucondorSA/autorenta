...
export class AuditClient {
     ...
     */
    async auditSecurityDefinerFunctions() {
     ...
     */
    async querySecurityDefinerFunctions() {
     ...
     */
    calculateFunctionRisk(functionName) {
    ...
    async auditRLSPolicies() {
        ...
        for (const table of (tables || [])) {
            ...
            policies.push({
                ...
                policy_names: (policyNames || []).map((p) => p.policyname)
            });
        }
        return policies;
    }
     ...
     */
    async findHighSeqScans(threshold = 1000) {
     ...
     */
    async queryHighSeqScansManually(threshold) {
     ...
     */
    generateRLSPolicyBoilerplate(tableName, userId = 'auth.uid()') {
