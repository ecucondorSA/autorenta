...
export function registerTools(server, supabase, audit) {
    ...
    server.registerTool('reject_booking', async (args) => {
        const schema = z.object({
    ...
    server.registerTool('block_car_availability', async (args) => {
        const schema = z.object({
            carId: z.string().uuid(),
        ...
        const result = await supabase.createCarAvailabilityBlock(carId, startDate, endDate, reason);
        return {
            success: true,
    ...
    server.registerTool('find_user', async (args) => {
        const schema = z.object({
    ...
    server.registerTool('check_car_availability', async (args) => {
        const schema = z.object({
            carId: z.string().uuid(),
            startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
            endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/)
    ...
    // Tool: Limpiar caché
    server.registerTool('clear_cache', async (args) => {
        const schema = z.object({
