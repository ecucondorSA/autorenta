...
export function registerResources(server: MCPServer, supabase: SupabaseClient) {
    ...
    async (params: any) => {
      ...
      return {
        car: {
          ...
          photos: car.car_photos?.map((p: any) => ({
            url: p.url,
            is_cover: p.is_cover
          })),
          owner: {
  ...
  server.registerResource(
    ...
    async () => {
      ...
      const todayBookings = bookingsToday.filter((b: any) =>
      ...
      return {
        ...
        summary: {
          ...
          active_bookings: bookingsToday.filter((b: any) => b.status === 'active').length,
          pending_approvals: bookingsToday.filter((b: any) => b.status === 'pending').length,
          platform_stats: stats,
          alerts: [
            todayBookings.length > 10 && 'Alto volumen de reservas hoy',
            bookingsToday.filter((b: any) => b.status === 'pending').length > 5 &&
  ...
  server.registerResource(
    ...
    async (params: any) => {
      ...
      if (year) {
        cars = cars.filter((c: any) => c.year === year);
      }

      if (minPrice) {
        cars = cars.filter((c: any) => c.daily_rate >= minPrice);
      }

      if (maxPrice) {
        cars = cars.filter((c: any) => c.daily_rate <= maxPrice);
      }
