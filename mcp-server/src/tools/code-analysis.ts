...
/**
 * Code Analysis & Fix Tools for MCP Server
 *
 * Provides automated code analysis, bug detection, and automatic fixing capabilities.
 * Includes integration with ESLint, TypeScript compiler, and custom bug detection.
 */
 ...
 */
function execCommand(command: string, cwd: string = '/home/edu/autorenta'): string {
  try {
 ...
 */
function parseTypeScriptOutput(output: string): CodeIssue[] {
  const issues: CodeIssue[] = [];
 ...
 */
function parseKnownBugs(filePath: string = '/home/edu/autorenta/BUGS_AUDIT_REPORT.md'): Array<{
  id: string;
  title: string;
  severity: string;
  location: string;
  ...
  try {
    ...
    while ((match = bugRegex.exec(content)) !== null) {
      ...
      const section = content.substring(
        startIndex,
        nextBugIndex === -1 ? content.length : nextBugIndex
      );
...
export function registerCodeAnalysisTools(server: MCPServer) {
  // ============================================================================
  ...
  server.registerTool(
    ...
    async (args: any) => {
      ...
      const changes: { file: string; changes_made: string[] }[] = [];
