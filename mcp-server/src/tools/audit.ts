...
export function registerAuditTools(server: MCPServer, audit: AuditClient) {
  ...
  server.registerTool(
    ...
    async (args: any) => {
      ...
      const riskLevels: Record<string, number> = { critical: 3, high: 2, medium: 1 };
  ...
  server.registerTool(
    ...
    async (args: any) => {
      ...
      const priorityActions = [
        ...report.critical_functions.map((f: any) => ({
          priority: 'CRITICAL',
          item: `Audit function: ${f.function_name}`,
          category: 'Security'
        })),
        ...report.rls_tables_without_policies.map((t: any) => ({
          priority: 'HIGH',
          item: `Create RLS policies for: ${t.table_name}`,
          category: 'Security'
        })),
        ...report.high_seq_scans.map((s: any) => ({
          priority: 'HIGH',
